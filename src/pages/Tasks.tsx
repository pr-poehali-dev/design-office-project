...
function CalendarView({ tasks }: { tasks: Task[] }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-5">
      ...
      <div className="flex-1 bg-white rounded-2xl border border-border p-5">
        ...
        <div className="flex items-center justify-between mb-5">
          <button onClick={prevMonth} className="p-2 rounded-xl hover:bg-muted transition-colors">
            <Icon name="ChevronLeft" size={18} className="text-stone" />
          </button>
          <span className="font-semibold text-stone">{MONTHS[month]} {year}</span>
          <button onClick={nextMonth} className="p-2 rounded-xl hover:bg-muted transition-colors">
            <Icon name="ChevronRight" size={18} className="text-stone" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-7 mb-2">
          {DAYS.map(d => (
            <div key={d} className="text-center text-xs text-stone-light font-medium py-1">{d}</div>
          ))}
        </div>
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center py-1.5 rounded-xl transition-all ${
                  ...
                  isToday ? "border-2 border-terra text-terra" :
                ...
                {dayTasks.length > 0 && (
                  <div className="flex gap-0.5 mt-0.5">
                    {dayTasks.slice(0, 3).map((t, j) => (
                      <span
                        key={j}
                        className={`w-1.5 h-1.5 rounded-full ${isSelected ? "bg-white/70" : PRIORITY_CONFIG[t.priority].dot}`}
                      />
                    ))}
                  </div>
                )}
      ...
      {/* Day tasks panel */}
      <div className="lg:w-72 bg-white rounded-2xl border border-border p-5">
        <h3 className="font-semibold text-stone text-sm mb-4">
          {selectedDay ? `${selectedDay} ${MONTHS[month].toLowerCase()}` : "Выберите день"}
        </h3>
        {todayTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            <Icon name="CalendarCheck" size={28} className="text-stone-light mb-2" />
            <p className="text-xs text-stone-light">Задач на этот день нет</p>
          </div>
        ) : (
          <div className="space-y-2.5">
            {todayTasks.map(t => (
              <div key={t.id} className="border border-border rounded-xl p-3 hover:border-terra/30 transition-colors">
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <span className="text-sm font-medium text-stone leading-snug">{t.title}</span>
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 mt-1 ${PRIORITY_CONFIG[t.priority].dot}`} />
                </div>
                <span className={`inline-block text-xs px-2 py-0.5 rounded-full border font-medium ${PROJECT_COLORS[t.project] || "bg-muted text-stone-mid border-border"}`}>
                  {t.project}
                </span>
                <div className="flex items-center gap-1 mt-1.5">
                  <span className="text-xs text-stone-light">{STATUS_LABELS[t.status]}</span>
                </div>
              </div>
            ))}
          </div>
