      ...
      <div className="lg:w-72 flex-shrink-0 bg-white rounded-2xl border border-border p-4">
        ...
        <div className="grid grid-cols-7 gap-0.5">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button key={day} onClick={() => setSelectedDay(day)} className={`relative flex flex-col items-center py-1 rounded-lg transition-all ${isSelected ? "terra-gradient text-white" : isToday ? "border border-terra text-terra" : "hover:bg-muted text-stone"}`}>
                <span className="text-xs font-medium leading-tight">{day}</span>
                {dayTasks.length > 0 && (
                  <div className="flex gap-px mt-0.5">
                    {dayTasks.slice(0, 3).map((t, j) => (
                      <span key={j} className={`w-1 h-1 rounded-full ${isSelected ? "bg-white/70" : PRIORITY_CONFIG[t.priority]?.dot || "bg-gray-400"}`} />
                    ))}
                  </div>
                )}
              </button>
        ...
        <div className="mt-4 pt-3 border-t border-border flex flex-wrap gap-2">
          {[{ dot: "bg-red-500", label: "Высокий" }, { dot: "bg-amber-400", label: "Средний" }, { dot: "bg-green-400", label: "Низкий" }].map(l => (
      ...
      <div className="flex-1 bg-white rounded-2xl border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-semibold text-stone">
              {selectedDay ? `${selectedDay} ${MONTHS[month]}, ${year}` : "Выберите день в календаре"}
            </h3>
            {todayTasks.length > 0 && (
          ...
          {selectedDay && (
            <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${selectedDay === today.getDate() && month === today.getMonth() ? "bg-terra-pale text-terra border-terra/20" : "bg-muted text-stone-mid border-border"}`}>
        ...
        {todayTasks.length === 0 ? (
          ...
          <div className="space-y-3">
            {todayTasks.map(t => (
              <div key={t.id} className="border border-border rounded-2xl p-4 hover:border-terra/30 hover:shadow-sm transition-all cursor-pointer group">
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className="text-xs px-2.5 py-1 rounded-full border font-medium bg-muted text-stone-mid border-border">{t.project_title || "—"}</span>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-xs text-stone-light">
                    ...
                    <span>Дедлайн: {formatDeadline(t.deadline)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-stone-light">{t.assigned_first_name || ""}</span>
