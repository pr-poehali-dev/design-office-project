...
export default function ProfilePage() {
  ...
  const startEditContacts = () => {
    setContactForm({ phone: user?.phone || "", city: user?.city || "", telegram: user?.telegram || "", website: user?.website || "" });
    setEditingContacts(true);
  };
  const saveContacts = () => { save(contactForm); setEditingContacts(false); };

  const startEditStyles = () => {
    setEditStyles(styles.length ? styles : []);
    setEditObjects(objects.length ? objects : []);
  ...
  const startEditHeader = () => {
    setHeaderForm({ first_name: user?.first_name || "", last_name: user?.last_name || "", experience_years: String(expYears) });
      ...
      {activeTab === "card" && (
        <div className="space-y-4">
            ...
            {editingHeader ? (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-stone-light mb-1 block">Имя</label>
                    <input value={headerForm.first_name} onChange={e => setHeaderForm(f => ({ ...f, first_name: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20" />
                  </div>
                  <div>
                    <label className="text-xs text-stone-light mb-1 block">Фамилия</label>
                    <input value={headerForm.last_name} onChange={e => setHeaderForm(f => ({ ...f, last_name: e.target.value }))} className="w-full px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20" />
                  </div>
                </div>
                <div>
                  <label className="text-xs text-stone-light mb-1 block">Опыт (лет)</label>
                  <input type="number" value={headerForm.experience_years} onChange={e => setHeaderForm(f => ({ ...f, experience_years: e.target.value }))} className="w-32 px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20" />
                </div>
          ...
          <div className="bg-white rounded-2xl border border-border p-5">
            ...
            {editingAbout ? (
              <div className="space-y-3">
                <textarea value={editBio} onChange={e => setEditBio(e.target.value)} rows={3} className="w-full px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20 resize-none" placeholder="Расскажите о себе..." />
                <div className="flex gap-2">
            ...
            {editingStyles ? (
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {ALL_STYLES.map(s => (
                      <button key={s} onClick={() => toggleTag(s, editStyles, setEditStyles)} className={`text-xs px-2.5 py-1 rounded-full border font-medium transition-all ${editStyles.includes(s) ? "terra-gradient text-white border-transparent" : "border-border text-stone-mid hover:border-terra/40"}`}>{s}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {ALL_OBJECTS.map(o => (
                      <button key={o} onClick={() => toggleTag(o, editObjects, setEditObjects)} className={`text-xs px-2.5 py-1 rounded-full border font-medium transition-all ${editObjects.includes(o) ? "bg-blue-500 text-white border-transparent" : "border-border text-stone-mid hover:border-blue-300"}`}>{o}</button>
                    ))}
                  </div>
                </div>
                <div className="flex gap-2">
              ...
              <>
                <div className="mb-4">
                  <p className="text-xs text-stone-light mb-2">Стили работы</p>
                  <div className="flex flex-wrap gap-1.5">
                    {styles.length > 0 ? styles.map(s => <span key={s} className="text-xs px-2.5 py-1 rounded-full bg-terra-pale text-terra border border-terra/20 font-medium">{s}</span>) : <span className="text-xs text-stone-light italic">Не указаны</span>}
                  </div>
                </div>
                <div>
                  <p className="text-xs text-stone-light mb-2">Типы объектов</p>
                  <div className="flex flex-wrap gap-1.5">
                    {objects.length > 0 ? objects.map(o => <span key={o} className="text-xs px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200 font-medium">{o}</span>) : <span className="text-xs text-stone-light italic">Не указаны</span>}
                  </div>
                </div>
              </>
            )}
          ...
          <div className="bg-white rounded-2xl border border-border p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="flex items-center gap-1 text-xs text-terra hover:underline"><Icon name="Plus" size={12} /> Добавить работу</button>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1">
              {MOCK_PORTFOLIO.map(item => (
                <div key={item.id} className="flex-shrink-0 w-32">
                  <div className="h-24 bg-gradient-to-br from-terra-pale via-terra/10 to-stone/5 rounded-xl flex items-center justify-center mb-1.5"><Icon name="Image" size={22} className="text-terra/25" /></div>
            ...
            {editingContacts ? (
              <div className="space-y-3">
                <div className="grid sm:grid-cols-2 gap-3">
                  <div><label className="text-xs text-stone-light mb-1 block">Телефон</label><input value={contactForm.phone} onChange={e => setContactForm(f => ({ ...f, phone: e.target.value }))} placeholder="+7..." className="w-full px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20" /></div>
                  <div><label className="text-xs text-stone-light mb-1 block">Город</label><input value={contactForm.city} onChange={e => setContactForm(f => ({ ...f, city: e.target.value }))} placeholder="Москва" className="w-full px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20" /></div>
                  <div><label className="text-xs text-stone-light mb-1 block">Telegram</label><input value={contactForm.telegram} onChange={e => setContactForm(f => ({ ...f, telegram: e.target.value }))} placeholder="@username" className="w-full px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20" /></div>
                  <div><label className="text-xs text-stone-light mb-1 block">Сайт / Behance</label><input value={contactForm.website} onChange={e => setContactForm(f => ({ ...f, website: e.target.value }))} placeholder="https://..." className="w-full px-3 py-2 rounded-xl border border-border bg-background text-stone text-sm focus:outline-none focus:ring-2 focus:ring-terra/20" /></div>
                </div>
                <div className="flex gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 animate-scale-in">
            ...
            <div className="text-center mb-4">
              ...
              <p className="text-xs text-stone-light mt-1">{user?.projects_count || 0} проектов &bull; {user?.city || "—"} &bull; {expYears > 0 ? `${yearsLabel(expYears)} опыта` : "—"}</p>
            </div>
